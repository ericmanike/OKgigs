...
const navItems = [
  ...
  { href: "/moderator/orders", label: "Orders", icon: ShoppingBag, exact: false },
];
    ...
    <div className="flex flex-col md:flex-row md:pt-28 pt-24 z-0 min-h-screen bg-zinc-50">
      ...
      >
        ...
        <div className="flex items-center justify-between mb-2 md:hidden pb-4 border-b border-zinc-100">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-zinc-700">Moderator Panel</span>
          </div>
          <button
            type="button"
            onClick={() => setSidebarOpen(false)}
        ...
        <div className="hidden md:flex items-center gap-2 px-3 py-2 mb-2">
          ...
          <span className="text-xs font-bold text-slate-600 uppercase tracking-wider">Moderator Panel</span>
        </div>
        ...
        <div className="flex flex-col gap-1 pt-1 md:pt-0">
          ...
          {role === 'admin' && (
            <Link
              href="/admin"
              className="flex items-center gap-3 px-3 py-2 rounded-xl text-[13px] font-medium text-purple-600 hover:bg-purple-50 transition-all duration-200 mb-2"
            >
              <span className="flex items-center justify-center w-8 h-8 rounded-lg bg-purple-100 shrink-0">
      ...
      <div className="flex-1 w-full md:pl-56 lg:pl-60 min-w-0">
        ...
        <div className="flex items-center gap-3 p-4 md:px-8 md:pt-6 border-b border-zinc-200 bg-white">
          ...
          <div>
            <h1 className="text-xl font-bold text-zinc-900">Moderator Dashboard</h1>
            <p className="text-zinc-500 text-xs mt-0.5">Manage daily operations and orders.</p>
          </div>
          <div className="ml-auto flex items-center gap-2">
            <div className="px-3 py-1.5 bg-slate-100 border border-slate-200 rounded-lg flex items-center gap-1.5 text-xs text-slate-700 font-medium">
              <Shield size={13} /> {role === 'admin' ? 'Admin' : 'Moderator'} Access
            </div>
          </div>
        </div>
